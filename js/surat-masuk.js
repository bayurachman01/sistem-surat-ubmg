...
function bindEvents() {
    // Tombol tambah surat
    var btnTambah = document.getElementById("btn-tambah-surat");
    if (btnTambah) {
    ...
    // Form submit
    var formSuratMasuk = document.getElementById("form-surat-masuk");
    if (formSuratMasuk) formSuratMasuk.addEventListener("submit", handleFormSubmit);

    // Tombol batal di form
    var btnFormBatal = document.getElementById("btn-form-batal");
    if (btnFormBatal) {
        btnFormBatal.addEventListener("click", function() {
    ...
    if (btnConfirmDelete) btnConfirmDelete.addEventListener("click", handleDelete);
...
async function loadSuratMasuk(filters) {
    ...
    if (pathname.includes("kotak-masuk.html")) {
        // Hanya yang diteruskan ke role user dan belum diarsipkan
        allSuratMasuk = rawData.filter(function(item) {
            return item["Diteruskan Ke"] === userRole && item["Status Baca"] !== "Sudah Diarsipkan";
        });
    } else if (pathname.includes("arsip.html")) {
        // Hanya yang diteruskan ke role user dan sudah diarsipkan
        allSuratMasuk = rawData.filter(function(item) {
            return item["Diteruskan Ke"] === userRole && item["Status Baca"] === "Sudah Diarsipkan";
        });
    } else {
        allSuratMasuk = rawData;
    }
            ...
            html += '<tr>' +
                ...
                '<td>' +
                    '<div class="flex gap-8">' +
                        (window.location.pathname.includes("kotak-masuk.html") || window.location.pathname.includes("arsip.html") ? 
                            '<button class="btn btn-sm btn-outline-primary" onclick="openDisposisiModal(\'' + item["ID"] + '\')" title="Tindak Lanjut">Tindak Lanjut</button>'
                        :
...
function openFormModal(id) {
    ...
    if (id) {
        ...
        var elDiteruskan = document.getElementById("diteruskan-ke");
        if (elDiteruskan) elDiteruskan.value = item["Diteruskan Ke"] || "";
...
async function handleFormSubmit(e) {
    ...
    var payload = {
        ...
        diteruskanKe:    elDiteruskan ? elDiteruskan.value : "",
...
function openDisposisiModal(id) {
    ...
    document.getElementById("disp-perihal").textContent = item["Perihal"] || "-";
    document.getElementById("disp-pengirim").textContent = item["Pengirim"] || "-";
    
    var elLampiran = document.getElementById("disp-lampiran");
    if (item["Link Lampiran"] && item["Link Lampiran"].toString().startsWith("http")) {
        elLampiran.innerHTML = '<a href="' + item["Link Lampiran"] + '" target="_blank" style="color:var(--color-primary);">Lihat Surat</a>';
    } else if (item["File Upload"] && item["File Upload"].toString().startsWith("http")) {
        elLampiran.innerHTML = '<a href="' + item["File Upload"] + '" target="_blank" style="color:var(--color-primary);">Lihat File</a>';
    } else {
        elLampiran.textContent = "-";
    }

    document.getElementById("disp-catatan").value = item["Catatan Rektor"] || "";
    document.getElementById("disp-status-baca").value = item["Status Baca"] === "Sudah Diarsipkan" ? "Sudah Diarsipkan" : "Sudah Dibaca";

    openModal("modal-disposisi");
}

document.addEventListener("DOMContentLoaded", function() {
    var formDisp = document.getElementById("form-disposisi");
    if (formDisp) {
        formDisp.addEventListener("submit", async function(e) {
            e.preventDefault();
            if (!editingId) return;

            var btnSimpan = document.getElementById("btn-disp-simpan");
            btnSimpan.disabled = true;
            btnSimpan.innerHTML = '<span class="spinner-inline"></span> Menyimpan...';

            var payload = {
                id: editingId,
                catatanRektor: document.getElementById("disp-catatan").value.trim(),
                statusBaca: document.getElementById("disp-status-baca").value
            };

            // Jika diarsipkan, ubah status utama ke Selesai
            if (payload.statusBaca === "Sudah Diarsipkan") {
                payload.status = "Selesai";
            }

            var result = await apiUpdateDisposisi(payload);

            btnSimpan.disabled = false;
            btnSimpan.innerHTML = 'Simpan Tindakan';

            if (result.success) {
                showToast(result.message || "Disposisi berhasil diperbarui.", "success");
                closeModal("modal-disposisi");
                loadSuratMasuk();
            } else {
                showToast(result.message || "Gagal menyimpan data.", "error");
            }
        ...
        document.getElementById("btn-disp-batal").addEventListener("click", function() {
            closeModal("modal-disposisi");
        });
    }
});
